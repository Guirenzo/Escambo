import { Gavel, ImageOff, RotateCcw, ShieldX } from 'lucide-react';
...
const COPY: Record<AppealDecision, { title: string; confirm: string; hint: string }> = {
  overturn: {
    title: 'Reverter remoção',
    confirm: 'Reverter e devolver a imagem',
    hint: 'A imagem volta para onde estava, se o lugar continua vazio, sai da lista de bloqueio e a remoção deixa de contar para a reincidência. O dono recebe a sua nota.',
  },
  uphold: {
    title: 'Manter remoção',
    confirm: 'Manter remoção',
    hint: 'A imagem continua fora do ar, o arquivo guardado é apagado e a remoção continua contando para a reincidência. O dono recebe a sua nota.',
  },
};
...
function QuarantineImage({ appeal, large = false }: { appeal: AdminAppeal; large?: boolean }) {
  ...
  const restored =
    !large && !appeal.hasImage && appeal.status === 'overturned' && !broken
      ...
      toast.success(
        r.status === 'upheld'
          ...
          : r.imageRestored
            ...
            : 'Remoção revertida. A imagem não foi recolocada: já havia outra no lugar ou o arquivo não existe mais.',
      );
      setDeciding(null);
    } catch (er) {
  ...
  return (
    <section className="card wide" aria-labelledby="appeals-title" data-testid="appeals-card">
      ...
      <p className="muted tiny">
        O dono de uma imagem removida contesta pelo perfil, uma vez e dentro do prazo. Até a
        decisão, o arquivo fica guardado fora do ar.
      </p>
      <QueryState
      ...
      {deciding && (
        <Modal title={COPY[deciding.decision].title} onClose={() => setDeciding(null)}>
          <form className="stack" onSubmit={confirm}>
            ...
            <p className="muted tiny">{COPY[deciding.decision].hint}</p>
            ...
            >
              {decide.isPending ? 'Salvando…' : COPY[deciding.decision].confirm}
            </Button>
          </form>
        </Modal>
      )}
