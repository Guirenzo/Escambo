...
| GET·POST·DELETE     | `/api/notifications/push`                                                       | **(auth)** avisos push no navegador (ADR 52): chave VAPID, aparelhos ligados e, com `?endpoint=`, se a assinatura deste aparelho é desta conta; assina este aparelho (`endpoint`, `p256dh`, `auth`; ligar e desligar ficam na auditoria só com o host); desliga pelo `endpoint`; `held` conta os avisos retidos pelo silêncio (ADR 54). `POST /api/notifications/push/test` manda um aviso de teste |
| GET · PUT           | `/api/notifications/preferences`                                                | **(auth)** preferências de aviso: e-mail, hora do resumo, fuso e a janela de silêncio do push (`quietHours {start, end}` ou null, ADR 54);  do dia (`digestHour`, 0 a 23, ADR 42) e fuso da conta (`timezone`, um dos fusos do Brasil, ADR 46): `instant` (por evento), `daily` (resumo diário) ou `off` (só senha e confirmação)                              |
...
| `quiet-push-summary` | Ao fim da janela de silêncio de cada pessoa ("não perturbe", ADR 54), no fuso dela, um push só com o que ficou retido: o próprio aviso, se foi um; um resumo com os títulos, se foram vários. Trava por marca d'água na conta: no máximo um, mesmo com duas instâncias. |
