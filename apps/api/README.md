...
| GET·POST·DELETE     | `/api/notifications/push`                                                       | **(auth)** avisos push no navegador (ADR 52): chave VAPID, aparelhos ligados e, com `?endpoint=`, se a assinatura deste aparelho é desta conta; assina este aparelho (`endpoint`, `p256dh`, `auth`); desliga pelo `endpoint`. `POST /api/notifications/push/test` manda um aviso de teste |
