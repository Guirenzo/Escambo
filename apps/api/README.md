...
| GET                 | `/api/services`                                                                 | Lista/busca serviços (`q`, `categoryId`, `isRemote`, `lat`/`lng`/`radiusKm`, `minPrice`/`maxPrice`, `maxDeliveryDays`, `minRating`, `sort=relevance\|price_asc\|price_desc\|rating\|newest\|distance`, `page`, `limit`)                                                                            |
