...
| POST                | `/api/auth/register`                                                            | Cria usuário (`email`, `password`, `role` e, opcional, `timezone` do aparelho, ADR 51)                                                                                                       |
...
| GET·POST·DELETE     | `/api/notifications/push`                                                       | **(auth)** avisos push no navegador (ADR 52): chave VAPID, aparelhos ligados e, com `?endpoint=`, se a assinatura deste aparelho é desta conta; assina este aparelho (`endpoint`, `p256dh`, `auth`; ligar e desligar ficam na auditoria só com o host); desliga pelo `endpoint`; `held` conta os avisos retidos pelo silêncio (ADR 54); `deliversWork` diz se a conta entrega trabalho e vê a escolha do que sai no silêncio (ADR 56). `POST /api/notifications/push/test` manda um aviso de teste |
| GET · PUT           | `/api/notifications/preferences`                                                | **(auth)** preferências de aviso: e-mail `instant` (por evento), `daily` (resumo diário) ou `off` (só senha e confirmação); hora do resumo do dia (`digestHour`, 0 a 23, ADR 42); fuso da conta (`timezone`, um dos fusos do Brasil, ADR 46); janela de silêncio do push (`quietHours {start, end}`, ou `null` para desligar, ADR 54); e o que sai durante ela (`quietPass`: hoje só `deadline`, o prazo vencido num trabalho que a pessoa entrega; `[]` = nada; no GET, `null` = nunca escolheu, que vale como nada; no PUT, sempre o conjunto inteiro, e `null` é recusado; ADR 56) |
...
| GET                 | `/api/admin/moderation/health`                                                  | **(admin)** saúde da moderação (ADR 47): fila e contestações esperando agora e, em `?days=` (1 a 365, padrão 30), tempo até decidir (mediana e p90), sinalizações automáticas por resultado e por sinal com o acerto, contestações e remoções por tipo, com a série por dia (Brasília, com as denúncias recebidas) e a meta de tempo `moderation_sla_hours` (ADR 50); na fila, quantas denúncias já passaram da meta e o estado do relatório diário (ADR 55) |
| GET                 | `/api/admin/moderation/health/export.csv?days=`                                 | **(admin)** série por dia da saúde da moderação em CSV (ADR 55): os últimos `days` dias inteiros de Brasília mais hoje — denúncias recebidas, sinalizações automáticas, decididas com ação e dispensadas, mediana de horas, meta e se passou dela; ponto e vírgula, vírgula decimal, BOM; ação registrada |
...
| `quiet-push-summary` | Ao fim da janela de silêncio de cada pessoa ("não perturbe", ADR 54), no fuso dela, um push só com o que ficou retido: o próprio aviso, se foi um; um resumo com os títulos, se foram vários. Trava por marca d'água na conta: no máximo um, mesmo com duas instâncias. O que saiu durante a janela por escolha da pessoa (ADR 56) não entra; os avisos de prazo vêm primeiro. |
| `purge-push-subscriptions` | Assinaturas de push sem nenhum aviso aceito pelo serviço de push há 180 dias são apagadas (a retenção que a Política de Privacidade promete). Pula com `PUSH_PROVIDER=off`. |
...
| `moderation-sla-report` | Relatório diário da meta da moderação (ADR 55): a partir de `DIGEST_HOUR` (Brasília), uma vez por dia, confere ontem e a fila; se a mediana de ontem passou de `moderation_sla_hours` ou alguma denúncia de conteúdo espera há mais que a meta, cada admin recebe um e-mail só com agregados e o link do painel. Marca do dia em `platform_settings` gravada antes do envio por compare-and-set (uma instância manda); até três tentativas no dia se o provedor não aceitar. Desliga com `moderation_sla_report_enabled`; pula com `MAIL_PROVIDER=off`. |
