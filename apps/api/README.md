...
| POST                | `/api/media`                                                                    | **(auth)** imagem para avatar ou portfólio: JPG, PNG, GIF ou WebP até 5 MB, reconhecida pelo conteúdo, sem EXIF/GPS; devolve a URL pública     |
| GET                 | `/api/media/:ano/:mês/:arquivo`                                                 | imagem pública de perfil ou portfólio, com cache imutável (fora do rate limit)                                                                  |
...
| POST · GET · PATCH · DELETE | `/api/saved-searches` · `/:id`                                           | **(auth)** buscas salvas: texto + filtros validados (até 20), PATCH renomeia e liga/desliga o alerta de serviço novo (ADR 35)          |
...
| `saved-search-alerts` | Buscas salvas com alerta, de hora em hora por busca: serviços criados desde o último aviso que casam com o texto e os filtros (fora os do próprio dono) viram uma notificação `saved_search_match` com até 5 títulos; o cursor avança mesmo sem resultado (ADR 35). |
