...
describe('savedSearchesService (ADR 35)', () => {
  it('create limpa texto e nome, serializa os filtros e devolve a linha gravada', async () => {
  ...
  it('filtros vazios viram null e o alerta começa desligado', async () => {
    await savedSearchesService.create(1, { query: 'logo', filters: {} });
    expect(repo.create).toHaveBeenCalledWith(
      expect.objectContaining({ filters: null, alertEnabled: false, name: null }),
    );
  });
  ...
  it('list lê filtros em string ou objeto; JSON quebrado vira null', async () => {
    ...
    expect(list[0]).toMatchObject({ alertEnabled: false, lastAlertAt: null });
  });
